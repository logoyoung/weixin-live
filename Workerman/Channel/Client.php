...
class Client 
{
    ...
    public static function connect($ip = '127.0.0.1', $port = 2206)
    {
        if(!self::$_remoteConnection)
        {
             ...
             self::$_remoteConnection->onClose = 'Workerman\Channel\Client::onRemoteClose'; 
             self::$_remoteConnection->onConnect = 'Workerman\Channel\Client::onRemoteConnect';
             self::$_remoteConnection->onMessage = 'Workerman\Channel\Client::onRemoteMessage';
             self::$_remoteConnection->connect();
             
             if(empty(self::$_pingTimer))
             {
                 self::$_pingTimer = Timer::add(self::$pingInterval, 'Workerman\Channel\Client::ping');
             }
         }    
    }
    ...
    public static function onRemoteClose()
    {
        ...
        self::clearTimer();
        self::$_reconnectTimer = Timer::add(1, 'Workerman\Channel\Client::connect', array(self::$_remoteIp, self::$_remotePort));
    }
