...
class Pdo{
     ...
     * @return unknown
     */
    public function query($sql, array $params, $debug = false, $mode = PDO::FETCH_ASSOC){
        $try = $this->try;
        do{
            try {
     ...
     * @return boolean
     */
    public function execute($sql, array $params, $debug = false){
        $try = $this->try;
        do{
            try {
                $this->connect(true);
    ...
    public function connect($master = false){
       ...
       try{
           ...
           $timer = new Timer();
           $timer->start();
           $this->PDOlinks[$linkId]= new PDO($dns, $config['dbuser'], $config['password'], self::$PDOoptions);
           $timer->end();
